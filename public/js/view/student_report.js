$(document).ready(async function () {
        ...
        buttons: [
            {
                ...
                messageTop: new Date().getFullYear() + '-' + ("0" + (new Date().getMonth() + 1)).slice(-2) + '-' + ("0" + new Date().getDate()).slice(-2),
    ...
    document.getElementById('search')
        .addEventListener('click', () => {
            ...
            if (!m && !f && !yn) {
                table.ajax.url('/admin/student/getall').load()
            }
            if (m) {
                const url = '/admin/student/get/' + m + '/null/' + (yn ? yn : 'null')
                table.ajax.url(url).load()
                console.log(url);
                return
            }
            if (f) {
                const url = '/admin/student/get/null/' + f + '/' + (yn ? yn : 'null')
                table.ajax.url(url).load()
                console.log(url);
                return
            }
            ...
            if (!f && !m && yn) {
                const url = '/admin/student/get/null/null/' + yn
                table.ajax.url(url).load()
                console.log(url);
                return
            }
